...
public class RoutingRequest implements Cloneable, Serializable {
    ...
    public void setIntermediatePlacesFromStrings(List<String> intermediates) {
    	String s = intermediates.get(0);
    	String[] places = s.split(";",-1);
        this.intermediatePlaces = new ArrayList<GenericLocation>(places.length);
        for (String place : places) {
            intermediatePlaces.add(GenericLocation.fromOldStyleString(place));
        }
    }
    
    public void setIntermediatePlacesWaitingTimesFromStrings(String intermediatesWT) {
    	String[] wt = intermediatesWT.split(",");
    	this.intermediatePlacesWaitingTimes = new ArrayList<String>(intermediatePlaces.size());
    	for(String w : wt)
    	intermediatePlacesWaitingTimes.add(w);
    }
